  ...
  for (const t of turnos) {
    ...
    if (isCanceled) {
      notifs.push({
        ...
        descripcion: isOwner
          ? `Un cliente canceló ${nombreServicio}.`
          : `Tu cita de ${nombreServicio} fue cancelada.`,
        fecha: t.fecha,
        hora: t.hora_inicio,
        leida: false,
        turnoId: t.id,
      ...
      notifs.push({
        ...
        descripcion: isOwner
          ? `Tienes agendado ${nombreServicio} hoy a las ${t.hora_inicio}.`
          : `Tienes ${nombreServicio} hoy a las ${t.hora_inicio}.`,
        fecha: t.fecha,
        hora: t.hora_inicio,
        leida: false,
        turnoId: t.id,
      ...
      notifs.push({
        ...
        descripcion: isOwner
          ? `${nombreServicio} en la agenda el ${formatNotifFecha(t.fecha)} a las ${t.hora_inicio}.`
          : `${nombreServicio} confirmada para el ${formatNotifFecha(t.fecha)} a las ${t.hora_inicio}.`,
        fecha: t.fecha,
        hora: t.hora_inicio,
        leida: false,
        turnoId: t.id,
      ...
      notifs.push({
        ...
        descripcion: isOwner
          ? `Solicitud de ${nombreServicio} el ${formatNotifFecha(t.fecha)} (${t.hora_inicio}).`
          : `${nombreServicio} el ${formatNotifFecha(t.fecha)} a las ${t.hora_inicio}.`,
        fecha: t.fecha,
        hora: t.hora_inicio,
        leida: false,
        turnoId: t.id,
...
export default function Header({ variant = "home" }: HeaderProps) {
  ...
  const fetchNotificaciones = useCallback(async () => {
    ...
    const sids = [
      ...new Set(turnos.map((t: any) => t.servicio_id).filter(Boolean)),
    ] as string[];
                      ...
                      {notificaciones.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
                          ...
                          <p className="text-xs text-gray-400 mt-1">
                            Tus citas y alertas aparecerán aquí
                          </p>
                        </div>
                      ) : (
                        notificaciones.map((notif) => (
                          ...
                          >
                            <span
                              className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mt-0.5 ${notifColorMap[notif.tipo]}`}
                            >
                            ...
                            {!notif.leida && (
                              <span className="flex-shrink-0 w-2 h-2 rounded-full bg-[var(--primary)] mt-2" />
                            )}
                          </div>
                        ))
                      )}
    ...
    >
      <div className="md:max-w-[1200px] mx-auto flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/register">
            <span
              className={`px-4 py-2 text-sm font-semibold rounded-xl transition-all duration-200 hover:brightness-110 hover:-translate-y-0.5
              ${
                solid
