...
import { IconEdit, IconCheck, IconX, IconMapPin, IconPhone, IconMail, IconCalendar, IconUser, IconInfoCircle, IconSettings, IconUpload, IconStar, IconStarFilled } from "@tabler/icons-react";
...
export default function UserData({
  ...
  const handleSaveProfile = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        ...(updatedAvatarUrl !== profilePicture && { avatar_url: updatedAvatarUrl }),
      })
      .eq("id", userId);
  ...
  const handleSubmitReview = async () => {
    ...
    if (reviewModal.rating === 0) {
      setReviewModal({ ...reviewModal, error: "Por favor, selecciona una calificación." });
      return;
    }
    ...
    if (error) {
       ...
       setReviewModal({ 
         ...reviewModal, 
         isSubmitting: false, 
         error: "Hubo un error al enviar tu reseña. El negocio o la tabla podría no estar completamente vinculada."
       });
    } else {
       setReviewModal({ ...reviewModal, isSubmitting: false, success: true });
       setTimeout(() => {
          setReviewModal({ isOpen: false, citaId: null, negocioId: null, rating: 0, comentario: "", isSubmitting: false, error: null, success: false });
          router.refresh();
       }, 2000);
    }
  };
                ...
                <div className="flex sm:justify-end">
                  {isEditing ? (
                    <div className="flex gap-2 w-full sm:w-auto mt-4 sm:mt-0">
                      <button 
                        onClick={() => { setIsEditing(false); setPreviewUrl(null); setFoto(null); setFormData({ nombre: name||"", telefono: phone||"", detalles: detalles||"", direction: direction||"" }); }}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-bold text-lg text-gray-900 flex items-center gap-2">
                   <IconCalendar size={20} className="text-[var(--primary)]"/> Historial de Citas
                </h2>
                {citas.length > 5 && (
                  <button className="text-sm text-[var(--primary)] hover:underline font-medium">Ver todas</button>
                )}
              </div>
              ...
              <div className="space-y-4">
                {citas.length > 0 ? (
                  citas.slice(0, 5).map((cita) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs md:text-sm text-gray-500 mt-0.5">
                            <span className="font-medium text-gray-700">{cita.servicio?.negocio?.nombre || "Establecimiento"}</span> •{" "}
                      ...
                      <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 w-full sm:w-auto mt-2 sm:mt-0 pt-2 sm:pt-0 border-t sm:border-0 border-gray-200">
                        ...
                        {(cita.estado === "pendiente" || cita.estado === "confirmado" || cita.estado === "confirmada") && (
                          <button
                            onClick={() =>
                              setCancelModal({
                                isOpen: true,
                        ...
                        {(cita.estado === "completada" || cita.estado === "completado" || cita.estado === "finalizado" || cita.estado === "finalizada") && (
                           <button
                             onClick={() =>
                               setReviewModal({
                                 isOpen: true,
                  ...
                  <div className="text-center py-10 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                    ...
                    <p className="text-sm text-gray-500 font-medium">Aún no tienes citas agregadas al historial.</p>
                  </div>
                )}
              </div>
            </div>
                    ...
                    {negocio.slice(0, 3).map((e) => (
                      ...
                      >
                        <div className="h-10 w-10 bg-gray-50 border border-gray-200 rounded-lg overflow-hidden flex-shrink-0 flex items-center justify-center">
                          {e.logo_url ? <img src={e.logo_url} alt="Logo" className="w-full h-full object-cover"/> : <span className="font-bold text-gray-400 text-xs">NG</span>}
                        </div>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl relative">
            ...
            {reviewModal.success ? (
              <div className="text-center py-6 animate-fade-in">
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-2">¡Gracias por tu reseña!</h3>
              ...
              <>
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-gray-900">Dejar Reseña</h3>
                    <p className="text-xs text-gray-500 mt-0.5">Evalúa tu experiencia en el establecimiento</p>
                  </div>
                </div>

                <div className="flex flex-col items-center mb-6">
                  <p className="text-sm font-medium text-gray-700 mb-3">¿Cómo calificarías tu cita?</p>
                  <div className="flex items-center gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button
                        key={star}
                        type="button"
                        onClick={() => setReviewModal({ ...reviewModal, rating: star, error: null })}
                        className={`transition-all duration-200 transform hover:scale-110 focus:outline-none`}
                      >
                        {reviewModal.rating >= star ? (
                          <IconStarFilled size={36} className="text-amber-400 drop-shadow-sm" />
                        ) : (
                          <IconStar size={36} className="text-gray-300 hover:text-amber-200" stroke={1.5} />
                        )}
                      </button>
                    ))}
                  </div>
                ...
                <div className="mb-6">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setReviewModal({ ...reviewModal, comentario: e.target.value })}
                ...
                <button
                  ...
                  disabled={reviewModal.isSubmitting || reviewModal.rating === 0}
